...
Global Enum $IT_NONE, $IT_LOCAL, $IT_URL, $IT_BINARY
; ===============================================================================================================================================================
...
;                          or raw binary data (mixed data is allowed).
...
Func _GUICtrlSlideshow_Create($hGUI, $iX, $iY, $iWidth, $iHeight, $avImage, $mOptions = Null)
    ...
    $mSlideshow['ErrorBkColor'] = $iErrorBkColor
...
Func __Slide($mSlideshow = Null, $iEvent = Null)
    ...
    Local $mCurrent, $avImage, $hBitmap, $hGraphics, $iCount, $hDisplayImage, $hPath, $iWidth, $iHeight, $iRadius
    Local $iPrevButtonPos, $iNextButtonPos, $hPen, $hFontFamily, $hFont, $hFormat, $tRect, $asCaptions, $nSlideStart, $vImage
    Local $hPrevBitmap, $hAttributes, $hTransition, $hTransitionGraphics, $tMatrix
    ...
    For $nIndex = $iStart To $iStop
        ...
        $vImage = $avImage[$mCurrent['Index']]
        $hBitmap = __CreateBitmapFromType($vImage, $mCurrent)
        ...
        EndIf
        _GDIPlus_GraphicsDrawImageRectRect($hGraphics, $hBitmap, 0, 0, $iWidth, $iHeight, 0, 0, $iWidth, $iHeight)
        ...
        If $mCurrent['Transition'] Then
            $hPrevBitmap = __CreateBitmapFromType($avImage[$mCurrent['PrevIndex']], $mCurrent)
            For $iOpacity = -1 To 0 Step (1 / $mCurrent['TransitionFrames'])
                ...
                _GDIPlus_GraphicsDrawImageRect($hTransitionGraphics, $hPrevBitmap, 0, 0, $iWidth, $iHeight)
            ...
            Next
            _GDIPlus_BitmapDispose($hPrevBitmap)
        EndIf
...
Func __GetLocalImage($sPath, $iWidth, $iHeight)
    Local $hImage, $hBitmap, $hGraphics, $aDim
...
Func __GetImageFromResource(ByRef $vImage, $iWidth, $iHeight, $bDownload = False)
    Local $hImage, $hBitmap, $hGraphics, $aDim, $dData
    If $bDownload Then
        $dData = InetRead($vImage, 1)
        If @error Then Return SetError(1, 0, False)
    Else
        $dData = $vImage
    EndIf
    $hImage = _GDIPlus_BitmapCreateFromMemory($dData)
...
Func __CreateBitmapFromType(ByRef $vImage, ByRef $mSlideshow)
    Local $hBitmap
    Local $iType = __GetImageType($vImage)
    Switch $iType
        Case $IT_NONE
            Return __NoImage($mSlideshow['Width'], $mSlideshow['Height'], $mSlideshow['ErrorBkColor'], $mSlideshow['ErrorColor'], $mSlideshow['ErrorFontName'], $mSlideshow['ErrorFontSize'])
        Case $IT_LOCAL
            $hBitmap = __GetLocalImage($vImage, $mSlideshow['Width'], $mSlideshow['Height'])
            If @error Then
                Return __NoImage($mSlideshow['Width'], $mSlideshow['Height'], $mSlideshow['ErrorBkColor'], $mSlideshow['ErrorColor'], $mSlideshow['ErrorFontName'], $mSlideshow['ErrorFontSize'])
            Else
                Return $hBitmap
            EndIf
        Case $IT_URL
            $hBitmap = __GetImageFromResource($vImage, $mSlideshow['Width'], $mSlideshow['Height'], True)
            If @error Then
                Return __NoImage($mSlideshow['Width'], $mSlideshow['Height'], $mSlideshow['ErrorBkColor'], $mSlideshow['ErrorColor'], $mSlideshow['ErrorFontName'], $mSlideshow['ErrorFontSize'])
            Else
                Return $hBitmap
            EndIf
        Case $IT_BINARY
            $hBitmap = __GetImageFromResource($vImage, $mSlideshow['Width'], $mSlideshow['Height'], False)
            If @error Then
                Return __NoImage($mSlideshow['Width'], $mSlideshow['Height'], $mSlideshow['ErrorBkColor'], $mSlideshow['ErrorColor'], $mSlideshow['ErrorFontName'], $mSlideshow['ErrorFontSize'])
            Else
                Return $hBitmap
...
Func __GetImageType(ByRef $vImage)
    If IsBinary($vImage) Then Return $IT_BINARY
    ; Strip leading and trailing spaces if the image is not raw binary
    $vImage = StringStripWS($vImage, 3)
    If FileExists($vImage) Then Return $IT_LOCAL
    If StringRegExp($vImage, '(?i)^(http(s)?:\/\/)?(www\.)?(.*?)$') Then Return $IT_URL
    Return $IT_NONE
EndFunc
